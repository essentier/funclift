...
class Traversable(Generic[F, A], Protocol):
    ...
    def traverse(
        self, f: Callable[[A], Applicative[AP, B]]
    ...
    def sequence(
        self, f_ap_a: Functor[F, Applicative[AP, A]]
    ) -> Applicative[AP, Functor[F, A]]:
...
class ListTraverser:
    ...
    def traverse(self, ls: list[A], f: Callable[[A], Applicative[AP, B]]) -> AP_LB:
...
class ListTraversable:
    ...
    @staticmethod
    def traverse(ls: list[A], f: Callable[[A], Applicative[AP, B]]) -> AP_LB:
        sig = signature(f)
    ...
    @staticmethod
    def traverse_right(
        mcls, ls: list[A], f: Callable[[A], Applicative[AP, B]]
    ) -> AP_LB:
    ...
    @staticmethod
    def traverse_left(mcls, ls: list[A], f: Callable[[A], Applicative[AP, B]]) -> AP_LB:
